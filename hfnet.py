...
def distancelessthresh(chose_2d, db_cand_p2d, thres):
    ...
    for i in range(len(db_cand_p2d)):
        ...
        diff = np.sqrt((chose_2d[0] - cand2d[0])**2+(chose_2d[1] - cand2d[1])**2)
        if (diff<dif_max):
            dif_max = diff
            indx = i
    if(dif_max<thres):
...
def baseline_sift_matching(img1, img2):
    ...
    img3 = cv2.drawMatchesKnn(img1, kp1, img2, kp2, good, None,
                              matchColor=(-1, -1, -1), matchesMask=None, # (0, 255, 0)
...
def debug_matching(frame1, frame2, path_image1, path_image2, result_dir, base_name1, base_name2, save_name, matches,
    ...
    if dst_h != h or dst_w != w:
        ...
        img_sift = cv2.resize(img_sift, (dst_w, dst_h), interpolation=cv2.INTER_AREA)

    cv2.imwrite(os.path.join(result_dir, base_name1 + '_' + base_name2 + '_' +  save_name + '_features.png'), img)
    cv2.imwrite(os.path.join(result_dir, base_name1 + '_' + base_name2 + '_sift.png'), img_sift)

    print(os.path.join(result_dir, base_name1 + '_' + base_name2 + '_' +  save_name + '_features.png'))
...
def match_frames(des1, des2, path_image1, path_image2, result_dir, base_name1, base_name2, save_name, num_points,
    ...
    if debug:
        debug_matching(frame1, frame2, path_image1, path_image2, result_dir, base_name1, base_name2, save_name, matches,
                       matches_mask, num_points, use_ratio_test, if_resize=True)
    ...
    if (model == 'hfnet'):
        ...
        with tf.Graph().as_default():
            ...
            with tf.Session(config=config) as sess:   
                ...
                # 定义输出的张量名称
                output_tensor_name = {n: sess.graph.get_tensor_by_name(n+':0')[0] for n in outputs} 
                #hf_keypoints_mat = []
                #hf_global_desc_mat = []
                ...
                if os.path.isfile(image_path) and '.txt' in image_path:
                    ...
                    lines = glob.glob(os.path.join(image_path, './**/*.jpg'))
                    lines=sorted(lines)
                else:
                ...
                for line in lines:
                    if(1):
                        ...
                        net_input = np.expand_dims(image[..., ::-1].astype(np.float), axis=0)
                        ...
                        hf_global_index_dict.append(database_img_n.split('data/')[-1])
                        hf_local_desc_mat.append(local_descriptors)
                        hf_keypoints_mat.append(keypoints)                    
                np.save(save_dir+prefix+'_'+'hfnet'+'_globaldesc.npy', np.array(hf_global_desc_mat,np.float32))
                np.save(save_dir+prefix+'_'+'hfnet'+'_globalindex.npy', hf_global_index_dict)
                print('saved global!')                
                np.save(save_dir+prefix+'_'+'hfnet'+'_localdesc.npy', hf_local_desc_mat)
                np.save(save_dir+prefix+'_'+'hfnet'+'_keypoints.npy', hf_keypoints_mat)
                print('saved all!')
...
if __name__ == '__main__':
    ...
    # image_path_ = '/home/ezxr/Documents/wxc/pic_ocr_flip'
    ...
    freeze_graph_images(pb_path=yolo_pb_path_, image_path=image_path_, save_dir=save_dir, prefix='test', model='hfnet')
