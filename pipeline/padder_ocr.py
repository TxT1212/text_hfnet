...
def ocr_images(input_path, imgs, output_path, chop_charactor):
    for img_path in tqdm(imgs):
        ...
        # print(str.replace(img_path, input_path, output_path)+".ocr_result.npy")
        np.save( str.replace(img_path, input_path, output_path)+".ocr_result.npy", np.array(result))
...
if __name__ == '__main__':
    ...
    if os.path.isdir(args.input_path):
        for dirpath, dirnames, filenames in os.walk(args.input_path):
            os.makedirs(dirpath.replace(args.input_path, args.output_path), exist_ok=True) 
            print(dirpath)
